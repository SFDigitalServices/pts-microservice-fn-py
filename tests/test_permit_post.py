...
# pylint: disable=redefined-outer-name,unused-argument,unused-import,line-too-long
import json
...
def test_permit_post_function(mock_get_oracle_connection, mock_env_access_key):
    ...
    mock_get_oracle_connection.return_value.cursor.return_value.__enter__.return_value.var.side_effect = Mock(
        side_effect=mock_side_effect
    )

    # Construct a mock HTTP request.
    req = func.HttpRequest(
        method="POST", headers=CLIENT_HEADERS, body=mock_body, url="/api/permit"
    )
...
@patch("permit_post.get_oracle_connection")
def test_permit_post_function_address_not_found(
    mock_get_oracle_connection, mock_env_access_key
):
...
def test_permit_post_oracle_error(mock_get_oracle_connection, mock_env_access_key):
    ...
    mock_get_oracle_connection.return_value.cursor.return_value.__enter__.return_value.callproc.side_effect = Exception(
        "oracle error"
    )
...
def test_permit_post_function_other(mock_env_access_key):
    ...
    # Construct a mock HTTP request.
    req = func.HttpRequest(
        method="POST", headers=CLIENT_HEADERS, body=None, url="/api/permit"
    )
...
def test_permit_post_function_request_error():
    ...
    with patch("permit_post.func_json_response") as mock:
        ...
        # Construct a mock HTTP request.
        req = func.HttpRequest(method="POST", body=None, url="/api/permit")
